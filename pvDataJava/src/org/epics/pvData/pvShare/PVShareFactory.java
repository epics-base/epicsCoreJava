...
public class PVShareFactory {
    public static PVField create(PVField pvNow,PVField pvShare) {
        PVField pvField = null;
        switch(pvField.getField().getType()) {
        case scalar:
            pvField = createScalar(pvNow.getParent(),(PVScalar)pvShare); break;
        case scalarArray:
            pvField = createArray(pvNow.getParent(),(PVArray)pvShare); break;
        case structure:
            // not legal to share PVStructure
            break;
        }
        return pvField;
    }
    
    private static PVField createScalar(PVStructure pvParent,PVScalar pvShare) {
    ...
    private static PVField createArray(PVStructure pvParent,PVArray pvShare) {
    ...
    private static class SharePVBooleanArrayImpl extends AbstractSharePVArray implements PVBooleanArray
    {
        ...
        public int put(int offset, int length, boolean[] from, int fromOffset) {
            ...
            try {
                ...
                if(number>0) {
                    if(super.getCapacity()!=pvShare.getCapacity()) {
                        super.capacity = pvShare.getCapacity();
                    }
                    if(super.getLength()!=pvShare.getLength()) {
                        super.setLength(pvShare.getLength());
                    }
                    super.postPut();
                }
